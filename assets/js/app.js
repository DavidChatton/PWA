  ...
  async handleCreateNote(noteContent) {
    try {
      ...
      if (!app.isOnline()) {
        ...
        newNote = {
          id: Date.now(), // Générer un ID unique pour la note
          note: noteContent,
          isNew: true
        };
        
        console.log("Notes sauvegardées localement :", savedNotes);
        app.setInLocalStorage("savedNotes", savedNotes);
        console.log(localStorage.getItem("savedNotes"))
        app.addNoteToUI(newNote); // Ajouter la note à l'interface utilisateur
        app.logOperation("Note ajoutée en étant hors ligne.");
      } else {
